...
class Game {
  constructor() {
    ...
    this.itemsInPlay = [new WeaponItem(600, 25, Game.DIM_X, Game.DIM_Y - Game.FLOOR_HEIGHT, "forceFieldBlast"),new Pig(Math.floor(Math.random() * 500), Math.floor(Math.random() * 300), Game.DIM_X, Game.DIM_Y - Game.FLOOR_HEIGHT)];
...
// NOTE: General flow functions
  ...
  checkSphereSphereCollisions(ctx) {
    ...
    let sphericalObjects = this.pigs.concat(this.itemsInPlay);
  ...
  checkPigAndItemsInPlayPlatformCollision() {
    let objects = this.pigs.concat(this.itemsInPlay);
      ...
      if (this.pigs[i].status === "available") {
        ...
        if (this.llama.forceFieldOn === true) {
          if (this.areSpheresColliding(this.llama.forceField, this.pigs[i])) {
            ...
            //pig was last
            this.itemsInPlay.push(new Bacon(this.pigs[i].x - this.pigs[i].radius, this.pigs[i].y - this.pigs[i].radius, Game.DIM_Y - Game.FLOOR_HEIGHT));
  ...
  checkLlamaItemsInPlayCollision() {
    for (let i = 0; i < this.itemsInPlay.length; i++) {
      ...
      if (this.areSphereNonSphereColliding(this.itemsInPlay[i], this.llama)) {
        if(this.itemsInPlay[i] instanceof Heart && this.itemsInPlay[i].status === "available") {
          // this.itemsInPlay.splice(i, 1);
          this.resolveLlamaItemsCollision(this.itemsInPlay[i]);
          this.llama.hearts++;
        } else if (this.itemsInPlay[i] instanceof Bacon && this.itemsInPlay[i].status === "available") {
          let bacon = this.itemsInPlay[i];
...
// NOTE: Miscellaneous functions
  ...
  allObjects() {
    let gameObjects = [].concat(this.pigs).concat(this.platforms).concat(this.itemsInPlay).concat(this.llama).concat(this.coins);
    return gameObjects;
  }

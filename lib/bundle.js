...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_10__sun_js__ = __webpack_require__(10);
...
class Game {
  constructor() {
    ...
    // this.itemsInPlay = [new WeaponItem(600, 25, Game.DIM_X, Game.DIM_Y - Game.FLOOR_HEIGHT, "forceFieldBlast"),new Pig(Math.floor(Math.random() * 500), Math.floor(Math.random() * 300), Game.DIM_X, Game.DIM_Y - Game.FLOOR_HEIGHT), new WeaponItem(600, 100, Game.DIM_X, Game.DIM_Y - Game.FLOOR_HEIGHT, "forceFieldBlast"), new WeaponItem(600, 50, Game.DIM_X, Game.DIM_Y - Game.FLOOR_HEIGHT, "forceFieldBlast") ];
    // this.itemsInPlay = [new WeaponItem(600, 300, Game.DIM_X, Game.DIM_Y - Game.FLOOR_HEIGHT, "bubbleShield"),
...
// NOTE: General flow functions
  ...
  manageWeaponItemsSpawing() {
    if (this.weaponsSpawnCycle < 900) { // 900/60 = 15 second cycle
  ...
  noMoreThanThreeWeaponsInPlay() {
    ...
    this.llama.weaponsInPossesion.forEach(weapon => {
  ...
  manageInactiveItems() {
    ...
    for (let i = 0; i < this.itemsInPlay.length; i++) {
      if (this.itemsInPlay[i] instanceof __WEBPACK_IMPORTED_MODULE_5__heart_js__["a" /* default */]) {
        ...
        if (this.itemsInPlay[i].status === "inactive") {
          if (this.itemsInPlay[i].x < - 50 || this.itemsInPlay[i].x > Game.DIM_X + 50) {
            this.itemsInPlay.splice(i, 1);
          }
        }
      }
  ...
  manageSpecialPigSpawning() {
    if (this.specialPigCycleSpawning < 120) { //a cycle of one minute 36000
  ...
  drawScore(ctx) {
    let outlineColor = this.background.luminary instanceof __WEBPACK_IMPORTED_MODULE_10__sun_js__["a" /* default */] ? "black" : "white"
  ...
  drawWeaponsContainer(ctx) {
    ...
    if (this.llama.weaponsInPossesion[0]) {
      let weapon = this.llama.weaponsInPossesion[0];
    ...
    if (this.llama.weaponsInPossesion[1]) {
      let weapon = this.llama.weaponsInPossesion[1];
      if (weapon instanceof __WEBPACK_IMPORTED_MODULE_8__weapons_forcefield_js__["a" /* default */]) {
    ...
    if (this.llama.weaponsInPossesion[2]) {
      let weapon = this.llama.weaponsInPossesion[2];
      if (weapon instanceof __WEBPACK_IMPORTED_MODULE_8__weapons_forcefield_js__["a" /* default */]) {
...
// NOTE: Miscellaneous functions
  ...
  returnCurrentActivatedWeapon() {
    for (let i = 0; i < this.llama.weaponsInPossesion.length; i++) {
      if (this.llama.weaponsInPossesion[i].status === "activated") {
        return this.llama.weaponsInPossesion[i];
      }
    }
  }

  returnEmptyWeaponSlot() {
    if (this.llama.weaponsInPossesion.length === 0) {
      return WEAPON1COORD;
    } else if (this.llama.weaponsInPossesion.length === 1) {
      return WEAPON2COORD;
    } else if (this.llama.weaponsInPossesion.length === 2) {
      return WEAPON3COORD;
    }
  }
  ...
  convertCollectedWeaponToLlamaWeapon(weaponType) {
    if (weaponType === "forceFieldBlast"){
      this.llama.weaponsInPossesion.push(new __WEBPACK_IMPORTED_MODULE_8__weapons_forcefield_js__["a" /* default */]());
    } else if (weaponType === "bubbleShield") {
      this.llama.weaponsInPossesion.push(new __WEBPACK_IMPORTED_MODULE_9__weapons_bubble_shield_js__["a" /* default */]());
    }
  }
  ...
  createNewSpecialPig() {
    let spawningCircle = this.returnEmptySpawnCircle();
    this.itemsInPlay.push(new __WEBPACK_IMPORTED_MODULE_3__pig_js__["a" /* default */](spawningCircle[0], spawningCircle[1] , Game.DIM_X, Game.DIM_Y - Game.FLOOR_HEIGHT, "invertedCommands"));
  }
  ...
  spawnNewWeaponItem() {
    ...
    if (this.llama.status === "normal") {
      ...
      this.itemsInPlay.push(new __WEBPACK_IMPORTED_MODULE_7__special_items_weapon_items_js__["a" /* default */](spawnX, spawnY, Game.DIM_X, Game.DIM_Y, randomDayWeapons[randomIndex]));
    } else if (this.llama.status === "flightMode") {
      let randomIndex = Math.floor(Math.random() * Math.floor(2));
      this.itemsInPlay.push(new __WEBPACK_IMPORTED_MODULE_7__special_items_weapon_items_js__["a" /* default */](spawnX, spawnY, Game.DIM_X, Game.DIM_Y, randomNightWeapons[randomIndex]));
    }
  ...
  allObjects() {
    let gameObjects = [].concat(this.platforms).concat(this.itemsInPlay).concat(this.llama).concat(this.coins);
    return gameObjects;
  }
...
class Llama {
  ...
  keyUpHandler(e) {
    ...
    if (e.keyCode === 37) {
      ...
      if(this.weaponsInPossesion.length > 0) {
  ...
  activateWeapon(weaponIndex) {
    ...
    for (let i = 0; i < this.weaponsInPossesion.length; i++) {
      if (this.weaponsInPossesion[i].status === "activated") {
        return;
      }
    }
  ...
  updateWeapons(ctx) {
    ...
    for (let i = 0; i < this.weaponsInPossesion.length; i++) {
      if (this.weaponsInPossesion[i].status === "inactive") {
        this.weaponsInPossesion.splice(i, 1);
      } else {
        this.weaponsInPossesion[i].update(ctx, this.x, this.y, this.x_velocity, this.y_velocity);
      }
    }
  ...
  returnCurrentActivatedWeapon() {
    for (let i = 0; i < this.weaponsInPossesion.length; i++) {
      if (this.weaponsInPossesion[i].status === "activated") {
        return this.weaponsInPossesion[i];
      }
    }
  }

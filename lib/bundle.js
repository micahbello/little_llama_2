...
const COINDEPOSITCOORD = [10, 5];
const HEARTDEPOSITCOORD = [70, 5];
...
// NOTE: General flow functions
  ...
  manageOutBoundsPigsAndItems() {
    ...
    for (let i = 0; i < this.specialObjects.length; i++) {
      if (this.specialObjects[i].x < - 50 || this.specialObjects[i].x > Game.DIM_X + 50) {
        this.specialObjects.splice(i, 1);
      }
    }

    for (let i = 0; i < this.coins.length; i++) {
      if (this.coins[i].x < - 50 || this.coins[i].x > Game.DIM_X + 50) {
  ...
  resolveLlamaItemsCollision(object) {
    ...
    if (object instanceof __WEBPACK_IMPORTED_MODULE_4__coin_js__["a" /* default */]) {
      destination.x = COINDEPOSITCOORD[0];
      destination.y = COINDEPOSITCOORD[1];
    } else if (object instanceof __WEBPACK_IMPORTED_MODULE_5__heart_js__["a" /* default */]) {
      destination.x = HEARTDEPOSITCOORD[0];
      destination.y = HEARTDEPOSITCOORD[1];
    }

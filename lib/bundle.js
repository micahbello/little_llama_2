...
class Game {
  constructor() {
    ...
    this.specialObjects = [new __WEBPACK_IMPORTED_MODULE_7__special_items_weapon_items_js__["a" /* default */](100, 100, Game.DIM_X, Game.DIM_Y - Game.FLOOR_HEIGHT, "forceFieldBlast")];
...
// NOTE: General flow functions
  ...
  manageInactiveItems() {
    ...
    for (let i = 0; i < this.specialObjects.length; i++) {
      ...
      } else if (this.specialObjects[i] instanceof __WEBPACK_IMPORTED_MODULE_6__bacon_js__["a" /* default */]) {
        if (this.specialObjects[i].status === "inactive") {
          let bacon = this.specialObjects[i];
  ...
  checkLlamaPigCollision() {
    ...
    for (let i = 0; i < this.pigs.length; i++) {
      ...
      if (this.pigs[i].status === "available") {
        ...
        if (this.llama.forceFieldOn === true) {
          if (this.areSpheresColliding(this.llama.forceField, this.pigs[i])) {
            ...
            //pig was last
            this.specialObjects.push(new __WEBPACK_IMPORTED_MODULE_6__bacon_js__["a" /* default */](this.pigs[i].x - this.pigs[i].radius, this.pigs[i].y - this.pigs[i].radius, Game.DIM_Y - Game.FLOOR_HEIGHT));
            if (this.pigs.splice(i, 1)) {
              continue;
            }
          }
        }
  ...
  checkLlamaSpecialObjCollision() {
    for (let i = 0; i < this.specialObjects.length; i++) {
      ...
      if (this.areSphereNonSphereColliding(this.specialObjects[i], this.llama)) {
        if(this.specialObjects[i] instanceof __WEBPACK_IMPORTED_MODULE_5__heart_js__["a" /* default */] && this.specialObjects[i].status === "available") {
          // this.specialObjects.splice(i, 1);
          this.resolveLlamaItemsCollision(this.specialObjects[i]);
          this.llama.hearts++;
        } else if (this.specialObjects[i] instanceof __WEBPACK_IMPORTED_MODULE_6__bacon_js__["a" /* default */] && this.specialObjects[i].status === "available") {
          let bacon = this.specialObjects[i];
          ...
          if ((this.llama.y < baconBottom && bacon.y < llamaBottom && llamaFront > bacon.x && baconFront > llamaFront) || (this.llama.y < baconBottom && bacon.y < llamaBottom && this.llama.x < baconFront && llamaFront > bacon.x)) {
            this.resolveLlamaItemsCollision(bacon);
            this.bacons ++;
          }
        }
      }
...
// NOTE: Miscellaneous functions
  ...
  areNoItemsInactive() {
    let objects = [].concat(this.coins).concat(this.specialObjects);

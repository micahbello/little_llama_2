...
class Llama {
  ...
  activateWeapon(weaponIndex) {
    if (this.weaponsInPossesion[weaponIndex] instanceof __WEBPACK_IMPORTED_MODULE_1__weapons_forcefield_js__["a" /* default */]) {
  ...
  updateWeapons(ctx) {
    for (let i = 0; i < this.weaponsInPossesion.length; i++) {
      if (this.weaponsInPossesion[i].status === "inactive") {
        ...
        this.weaponsInPossesion[i].update(ctx, this.x, this.y);
      }
    }

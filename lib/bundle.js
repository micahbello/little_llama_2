...
class Game {
  constructor() {
    ...
    this.platforms = [];
    this.specialObjects = [new __WEBPACK_IMPORTED_MODULE_5__heart_js__["a" /* default */](100, 200, Game.DIM_X, Game.DIM_Y - Game.FLOOR_HEIGHT)];
    // this.specialObjects = [];
...
//REDO THIS LATER FOR BETTER OPTOMIZATION
  ...
  areSphereNonSphereColliding(sphere, fourSidedObj) {
    let objFront = fourSidedObj.x + fourSidedObj.width;
    let objBottom = fourSidedObj.y + fourSidedObj.height;

    if (sphere.x > fourSidedObj.x && sphere.x < objFront && sphere.y > fourSidedObj.y && sphere.y < objBottom) {
      return true;
    }

...
class Game {
  constructor() {
    ...
    this.pigs = [new __WEBPACK_IMPORTED_MODULE_3__pig_js__["a" /* default */](20, 20, Game.DIM_X, Game.DIM_Y - Game.FLOOR_HEIGHT, "testing"), new __WEBPACK_IMPORTED_MODULE_3__pig_js__["a" /* default */](700, 20, Game.DIM_X, Game.DIM_Y - Game.FLOOR_HEIGHT, "testing"),
    new __WEBPACK_IMPORTED_MODULE_3__pig_js__["a" /* default */](Math.floor(Math.random() * 500), Math.floor(Math.random() * 300), Game.DIM_X, Game.DIM_Y - Game.FLOOR_HEIGHT)];
    this.platforms = [];
...
//comment in to test pig span circles
...
// pig related functions
  ...
  checkEmptySpawnCircle() {
//REDO THIS LATER FOR BETTER OPTOMIZATION

    let objects = [].concat(this.llama).concat(this.pigs);
  ...
  createNewPig() {
    let spawningCircle = this.checkEmptySpawnCircle();
...
//BELOW ARE COLLISION DETECTION RELATED FUNCTIONS
  checkObjectCollisions() {
    ...
    this.checkPigPlatformCollision();
    this.checkLlamaPigCollision();
  }

  checkPigPlatformCollision() {
    for (let i = 0; i < this.platforms.length; i++) {
      for (let j = 0; j < this.pigs.length; j++) {
        ...
        if (pig.status === "normal") {
          if (pig.x + pig.radius > platform.x && pig.x - pig.radius < platformFront && pig.y + pig.radius > platform.y && pig.y - pig.radius < platformBottom){

            if (pig.y_velocity === Math.abs(pig.y_velocity)) {
              pig.y = platform.y - pig.radius + 2
            } else {
              pig.y = platformBottom + pig.radius + 2
            }
      ...
      if (this.pigs[i].status === "normal") {
        ...
        if (this.pigs[i].x > this.llama.x && this.pigs[i].x < llamaFront && this.pigs[i].y > this.llama.y && this.pigs[i].y < llamaBottom) {
          ...
          if (this.shaking === false) {
            ...
            this.shakingTimer ++;
            this.llama.hearts --;
            this.pigs[i].status = "llamaCollided";
            this.thatsAPig.play();
          }
        }
      }

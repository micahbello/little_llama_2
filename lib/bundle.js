...
// NOTE: General flow functions
  ...
  checkLlamaSpecialObjCollision() {
    console.log(this.bacons);
    for (let i = 0; i < this.specialObjects.length; i++) {
      let llamaFront = this.llama.x + this.llama.width;
      let llamaBottom = this.llama.y + this.llama.height;

      if (this.areSphereNonSphereColliding(this.specialObjects[i], this.llama)) {
        if(this.specialObjects[i] instanceof __WEBPACK_IMPORTED_MODULE_5__heart_js__["a" /* default */] && this.specialObjects[i].status === "active") {
          // this.specialObjects.splice(i, 1);
          this.resolveLlamaItemsCollision(this.specialObjects[i]);
          this.llama.hearts++;
        } if (this.specialObjects[i] instanceof __WEBPACK_IMPORTED_MODULE_6__bacon_js__["a" /* default */] && this.specialObjects[i].status === "active") {
          let bacon = this.specialObjects[i];
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__forcefield_js__ = __webpack_require__(13);
...
class Llama {
  constructor(canvas_width, canvas_height) {
    ...
    this.forceField = new __WEBPACK_IMPORTED_MODULE_1__forcefield_js__["a" /* default */]();

...
class Game {
  constructor() {
    ...
    this.specialObjects = [new WeaponItem(100, 100, Game.DIM_X, Game.DIM_Y - Game.FLOOR_HEIGHT, "forceFieldBlast")];
...
// NOTE: General flow functions
  ...
  manageInactiveItems() {
    ...
    for (let i = 0; i < this.specialObjects.length; i++) {
      ...
      } else if (this.specialObjects[i] instanceof Bacon) {
        if (this.specialObjects[i].status === "inactive") {
      ...
      if (this.pigs[i].status === "available") {
        ...
        if (this.llama.forceFieldOn === true) {
          if (this.areSpheresColliding(this.llama.forceField, this.pigs[i])) {
            ...
            //pig was last
            this.specialObjects.push(new Bacon(this.pigs[i].x - this.pigs[i].radius, this.pigs[i].y - this.pigs[i].radius, Game.DIM_Y - Game.FLOOR_HEIGHT));
  ...
  checkLlamaSpecialObjCollision() {
    for (let i = 0; i < this.specialObjects.length; i++) {
      ...
      if (this.areSphereNonSphereColliding(this.specialObjects[i], this.llama)) {
        if(this.specialObjects[i] instanceof Heart && this.specialObjects[i].status === "available") {
          // this.specialObjects.splice(i, 1);
          this.resolveLlamaItemsCollision(this.specialObjects[i]);
          this.llama.hearts++;
        } else if (this.specialObjects[i] instanceof Bacon && this.specialObjects[i].status === "available") {
          let bacon = this.specialObjects[i];
